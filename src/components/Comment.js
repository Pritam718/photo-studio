...
const Comment = (props) => {
  ...
  const fetchComment=async()=>{
    const allcomments=await axios?.get(`/api/photo/getComment/${props.postId}`)
 ...
 useEffect(()=>{
  ...
  <div className="px-10">
    <div className="bg-white max-w-xl rounded-2xl px-10 py-8 shadow-lg hover:shadow-2xl transition duration-500">
      <div className="w-14 h-14 bg-yellow-500 rounded-full flex items-center justify-center font-bold text-white">flashes</div>
      <div className="mt-4">
        <h1>{props.like} likes</h1>
        <h1 className="text-lg text-gray-700 font-semibold hover:underline cursor-pointer">Comments</h1>
        <div className="  h-80 overflow-y-auto">
        {
          showComment.data!==undefined && showComment.data.data.length >0 && (
            <>
              {
                  ...
                  <div>
                    <div class="flex items-center space-x-2 mx-2 my-5">
                      <div className="flex flex-shrink-0 self-start cursor-pointer">
                            <img src="https://images.unsplash.com/photo-1551122089-4e3e72477432?ixid=MXwxMjA3fDB8MHxzZWFyY2h8M3x8cnVieXxlbnwwfHwwfA%3D%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=500&q=60" 
                            alt="" 
                      ...
                      <div className="bg-gray-100 w-auto rounded-xl px-5 pb-3">
                        <div className="font-semibold">
                          <a href="#" class="hover:underline text-xl">
                            <small>{comments.userId.firstName} {comments.userId.lastName}</small>
                          </a>
                        </div>
